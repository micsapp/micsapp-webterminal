...
const ABOUT = `mics_cli ${VERSION}  ·  build ${BUILD}
Command-line client for micsapp-webterminal — a browser-based multi-tenant
terminal with file management, split panes, and remote desktop access.
Source: https://github.com/micsapp/micsapp-webterminal`;
...
async function run(rawArgv) {
  ...
  // Bare `mics_cli` (no command, no flags) prints the help screen plus a
  // short about block so users see the version/build at a glance. Explicit
  // `help` / `--help` skips the about block to keep its output tight.
  if (!rawArgv.length) {
    out(HELP);
    out(ABOUT);
    return;
  ...
  if (argv[0] === '--version' || argv[0] === '-v' || argv[0] === 'version') {
    out(`mics_cli ${VERSION}  ·  build ${BUILD}`);
    return;
  }
